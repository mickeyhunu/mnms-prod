...
async function listAdmin() {
  const [rows] = await getPool().query(
    `SELECT c.id, c.store_no AS storeNo, c.worker_name AS workerName, c.content,
            ...
            MAX(CASE WHEN r.resolved_at IS NULL THEN r.created_at END) AS lastPendingReportedAt,
            MAX(r.resolved_at) AS lastResolvedAt
       FROM attendance_comments c
       JOIN users u ON u.id = c.user_id
      ...
      ORDER BY (pendingReportCount > 0) DESC,
               lastPendingReportedAt DESC,
               c.created_at DESC,
               c.id DESC`
  );
  return rows.map((row) => ({
