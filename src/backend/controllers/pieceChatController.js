...
function serializeRoom(room, messages, currentUserId) {
  ...
  return { post: room.post, lifecycle, ...serializeMembers(room, currentUserId), messages: [guideMessage, ...serializeMessages(messages, room.viewerRole), ...lifecycleMessages(lifecycle)] };
}
...
async function getMessages(req, res, next) { try { const value = await context(req, res); if (!value) return; const afterId = Math.max(0, Number.parseInt(req.query.afterId, 10) || 0); const lifecycle = resolvePieceChatLifecycle(value.room.post, new Date(), value.room.participants.length); res.json([...serializeMessages(await model.listMessagesAfter(value.postId, afterId), value.room.viewerRole), ...lifecycleMessages(lifecycle)]); } catch (e) { next(e); } }
