...
const { resolvePieceChatLifecycle, lifecycleMessages, sortPieceChatMessages } = require('../utils/pieceChatLifecycle');
...
function serializeMembers(room, currentUserId) {
  ...
  return { viewerRole: room.viewerRole, canManage: room.canManage, currentUserId, members, participants: room.participants, readStates: room.readStates };
}
function serializeRoom(room, messages, currentUserId) {
  ...
  const timeline = [guideMessage, ...serializeMessages(messages, room.viewerRole), ...lifecycleMessages(lifecycle)];
  return { post: room.post, lifecycle, ...serializeMembers(room, currentUserId), messages: sortPieceChatMessages(timeline) };
}
function serializeMessages(messages, viewerRole) {
  return messages.map((message) => message.isHidden && viewerRole !== 'ADMIN' ? { ...message, content: null } : message);
}
...
async function getMessages(req, res, next) { try { const value = await context(req, res); if (!value) return; const afterId = Math.max(0, Number.parseInt(req.query.afterId, 10) || 0); const lifecycle = resolvePieceChatLifecycle(value.room.post, new Date(), value.room.participants.length); const messages = [...serializeMessages(await model.listMessagesAfter(value.postId, afterId), value.room.viewerRole), ...lifecycleMessages(lifecycle)]; res.json(sortPieceChatMessages(messages)); } catch (e) { next(e); } }
async function getMessageVisibility(req, res, next) { try { const value = await context(req, res); if (!value) return; res.json(await model.listMessageVisibility(value.postId)); } catch (e) { next(e); } }
...
async function setMessageVisibility(req, res, next) { try { const value = await context(req, res); if (!value) return; if (value.room.viewerRole !== 'ADMIN') return res.status(403).json({ message: '관리자만 채팅 메시지를 제한할 수 있습니다.' }); const messageId = parseId(req.params.messageId); if (!messageId || typeof req.body.hidden !== 'boolean') return res.status(400).json({ message: '메시지 제한 상태를 확인해주세요.' }); const updated = await model.setMessageHidden(value.postId, messageId, req.user.id, req.body.hidden); if (!updated) return res.status(404).json({ message: '제한할 채팅 메시지를 찾을 수 없습니다.' }); res.json({ success: true, messageId, isHidden: req.body.hidden }); } catch (e) { next(e); } }
...
module.exports = { getRoom, getMembers, getMessages, getMessageVisibility, sendMessage, setMessageVisibility, markRead, getUnread, attendance, remove };
