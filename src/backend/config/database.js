...
async function initDatabase() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS business_ads (
      ...
      piece_chat_notice VARCHAR(500) NOT NULL DEFAULT '',
  ...
  const businessAdsColumnMigrations = [
    ...
    { name: 'piece_chat_notice', sql: "ALTER TABLE business_ads ADD COLUMN piece_chat_notice VARCHAR(500) NOT NULL DEFAULT '' AFTER telegram_id" },
    { name: 'show_business_address_map', sql: 'ALTER TABLE business_ads ADD COLUMN show_business_address_map TINYINT(1) NOT NULL DEFAULT 0 AFTER piece_chat_notice' },
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS stamp_histories (
      ...
      action_type ENUM('STAMP_PURCHASE','STAMP_PURCHASE_CANCEL','VISIT_VERIFICATION','SERVICE_BOTTLE_USE','BUSINESS_AD_BASIC','BUSINESS_AD_PLUS','BUSINESS_AD_PREMIUM','BUSINESS_AD_PIECE','ADMIN_ADJUST_ADD','ADMIN_ADJUST_DEDUCT','EXPIRED') NOT NULL,
      amount INT NOT NULL,
      reason VARCHAR(255) NULL,
