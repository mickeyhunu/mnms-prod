...
function loadKakaoShareSdk() {
   ...
   kakaoShareSdkLoader = new Promise((resolve, reject) => {
      ...
      if (existingScript) {
         ...
         if (existingScript.dataset.kakaoShareSdkLoaded === 'true' || existingScript.readyState === 'complete') {
            resolveKakaoShareSdk(resolve, reject);
            return;
         }

         existingScript.addEventListener('load', () => {
            existingScript.dataset.kakaoShareSdkLoaded = 'true';
            resolveKakaoShareSdk(resolve, reject);
         }, { once: true });
      ...
      script.addEventListener('load', () => {
         script.dataset.kakaoShareSdkLoaded = 'true';
         resolveKakaoShareSdk(resolve, reject);
      }, { once: true });
...
async function sendKakaoDefaultShare({ title, description, url, buttonTitle = '사이트 바로가기', imageUrl } = {}) {
   ...
   sendDefault.call(kakao.Share || kakao.Link, {
      objectType: 'feed',
      content: {
