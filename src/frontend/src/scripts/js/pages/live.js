    ...
    try {
        ...
        liveState.accessRules = {
            ...
            access: {
                ...
                mind: Boolean(response?.access?.entry)
            }
        };
    } catch (error) {
...
function renderCategoryButtons(categories) {
    ...
    if (!selectedHasAccess && !['entry', 'mind', 'chojoong', 'waiting'].includes(liveState.selectedCategoryKey)) {
    ...
    categoryFilter.innerHTML = normalizedCategories.map((category) => {
        ...
        const shouldShowLockedStyle = !hasAccess && !['chojoong', 'waiting', 'entry', 'mind'].includes(category.key);
...
function getLiveCategoryDeniedReason(categoryKey) {
    ...
    if (isInactiveBusinessMember && ['chojoong', 'waiting', 'entry', 'mind'].includes(categoryKey)) {
    ...
    if (categoryKey === 'entry' || categoryKey === 'mind') {
        const categoryLabel = categoryKey === 'mind' ? '마인드표' : '엔트리';
        if (level >= 4) {
            return '';
        }
        if (level < 3) {
...
function shouldSyncLatestCardViewport(categoryKey = liveState.selectedCategoryKey) {
    return Boolean(LIVE_CATEGORIES[categoryKey]) && categoryKey !== 'mind';
}
...
function isEntryRestrictedView() {
    return ['entry', 'mind'].includes(liveState.selectedCategoryKey)
...
function buildLiveEntriesQuery({ appendOlder = false } = {}) {
    return {
        ...
        limit: ['entry', 'mind'].includes(liveState.selectedCategoryKey) ? LIVE_ENTRY_PAGE_SIZE : LIVE_HISTORY_PAGE_SIZE,
