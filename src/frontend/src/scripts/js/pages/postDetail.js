...
function renderPieceParticipants(post, isHiddenPost) {
    ...
    participantsListElement.innerHTML = isPiecePost
        ? [
            ...
            ...participants.map((participant) => renderPieceParticipantCard(participant))
        ].join('')
        : '';
}
...
function renderPostDetail(post) {
    ...
    const boardNameMap = { FREE: '자유게시판', ANON: '익명게시판', REVIEW: '후기게시판', STORY: '썰게시판', PIECE: '조각게시판', ATTENDANCE: '출석게시판', QUESTION: '질문게시판', EVENT: '이벤트게시판', NEWS: '최신정보(뉴스)', PROMOTION: '홍보게시판' };
    const boardTagMap = { FREE: '자유', ANON: '익명', REVIEW: '후기', STORY: '썰', PIECE: '조각', ATTENDANCE: '출석', QUESTION: '질문', EVENT: '이벤트', NEWS: '최신정보(뉴스)', PROMOTION: '홍보' };
    const boardType = String(post.boardType || '').toUpperCase();
